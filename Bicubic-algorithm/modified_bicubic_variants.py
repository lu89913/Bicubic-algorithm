...
def cubic_keys(x, a):
    """
    Keys cubic kernel function.
    'a' is the sharpness parameter (e.g., -0.5, -0.75, -1.0).
    """
    x = np.array(x).astype(np.float64)
    absx = np.absolute(x)
    absx2 = np.multiply(absx, absx)
    ...
    # Formula from Wikipedia / general Keys definition
    # if |x| <= 1: (a+2)|x|^3 - (a+3)|x|^2 + 1
    # if 1 < |x| <= 2: a|x|^3 - 5a|x|^2 + 8a|x| - 4a

    term1 = np.multiply((a + 2.0) * absx3 - (a + 3.0) * absx2 + 1.0, absx <= 1.0)
    term2 = np.multiply(a * absx3 - 5.0 * a * absx2 + 8.0 * a * absx - 4.0 * a, (1.0 < absx) & (absx <= 2.0))
    f = term1 + term2
    return f

def cubic_mitchell_netravali(x, B, C):
    """
    Mitchell-Netravali cubic kernel function.
    Commonly B + 2C = 1. Mitchell & Netravali recommend B=1/3, C=1/3.
    """
    x = np.array(x).astype(np.float64)
    absx = np.absolute(x)
    absx2 = np.multiply(absx, absx)
    ...
    # Formula from Wikipedia / general graphics texts
    term1_factor1 = (12.0 - 9.0*B - 6.0*C)
    term1_factor2 = (-18.0 + 12.0*B + 6.0*C)
    term1_factor3 = (6.0 - 2.0*B)
    term1 = np.multiply((1.0/6.0) * (term1_factor1*absx3 + term1_factor2*absx2 + term1_factor3), absx < 1.0)

    term2_factor1 = (-B - 6.0*C)
    term2_factor2 = (6.0*B + 30.0*C)
    term2_factor3 = (-12.0*B - 48.0*C)
    term2_factor4 = (8.0*B + 24.0*C)
    term2 = np.multiply((1.0/6.0) * (term2_factor1*absx3 + term2_factor2*absx2 + term2_factor3*absx + term2_factor4), (1.0 <= absx) & (absx < 2.0))
...
# Renamed imresize to imresize_variant and added kernel selection logic
def imresize_variant(I, scalar_scale=None, method='bicubic', output_shape=None, mode="vec",
                     kernel_type='keys', kernel_param_a=-0.5,
                     kernel_param_b=1/3.0, kernel_param_c=1/3.0):

    selected_kernel_func = None
    # kernel_width will be set based on method (bicubic or bilinear)

    if method == 'bicubic':
        if kernel_type == 'keys':
            # The 'cubic' function in this file is Keys with a=-0.5.
            # We use cubic_keys to allow 'a' to be a parameter.
            selected_kernel_func = lambda x: cubic_keys(x, kernel_param_a)
            # print(f"Using Keys kernel with a={kernel_param_a}")
        elif kernel_type == 'mitchell_netravali':
            selected_kernel_func = lambda x: cubic_mitchell_netravali(x, kernel_param_b, kernel_param_c)
            # print(f"Using Mitchell-Netravali kernel with B={kernel_param_b}, C={kernel_param_c}")
        else:
            raise ValueError(f"Unknown kernel_type for bicubic method: {kernel_type}")
        kernel_width = 4.0 # Standard for cubic kernels
    elif method == 'bilinear':
        selected_kernel_func = triangle # triangle function is defined above
        kernel_width = 2.0
    else:
        raise ValueError(f'Unidentified interpolation method supplied: {method}')

    # Check if a kernel function was actually selected (it should always be by this point)
    if selected_kernel_func is None:
        raise RuntimeError("Kernel function was not selected. This should not happen.")
    ...
    for k_dim_idx in range(2):
        ...
        # Pass the dynamically selected kernel function to contributions
        w, ind_0based = contributions(dim_size_in, dim_size_out, dim_scale, selected_kernel_func, kernel_width)
...
if __name__ == '__main__':
    print("Running tests for modified_bicubic_variants.py...")
    ...
    # Ensure output directory exists (though "." is current dir)
    # import os
    ...
    try:
        ...
        test_configs = [
            {'kernel_type': 'keys', 'kernel_param_a': -0.5, 'name': 'Keys (a=-0.5, Traditional)'},
            {'kernel_type': 'keys', 'kernel_param_a': -0.75, 'name': 'Keys (a=-0.75)'},
            {'kernel_type': 'keys', 'kernel_param_a': -1.0, 'name': 'Keys (a=-1.0)'},
            {'kernel_type': 'mitchell_netravali', 'kernel_param_b': 1/3.0, 'kernel_param_c': 1/3.0, 'name': 'Mitchell-Netravali (B=1/3, C=1/3)'},
        ]

        for config in test_configs:
            print(f"\n--- Testing: {config['name']} ---")
            start_time = time.time()
            resized_img_array = imresize_variant(
                input_array,
                output_shape=target_shape,
                method='bicubic',
                mode='vec', # Using vec mode for performance
                kernel_type=config['kernel_type'],
                kernel_param_a=config.get('kernel_param_a'),
                kernel_param_b=config.get('kernel_param_b'),
                kernel_param_c=config.get('kernel_param_c')
            )
            end_time = time.time()

            exec_time = end_time - start_time
            print(f"Execution time: {exec_time:.4f} seconds")
            print(f"Resized image shape: {resized_img_array.shape}")

            psnr = calculate_psnr_arrays(golden_array, resized_img_array)
            if psnr is not None:
                print(f"PSNR vs Golden: {psnr:.4f} dB")

            # Sanitize name for filename
            filename_suffix = config['name'].lower().replace('(', '').replace(')', '').replace('=', '').replace('/', '_').replace(',', '').replace(' ', '_')
            output_image_path = f"{output_dir}/lena_variant_{filename_suffix}_512.png"

            if resized_img_array.shape == target_shape:
                Image.fromarray(resized_img_array.astype(np.uint8)).save(output_image_path)
                print(f"Saved result to {output_image_path}")
            else:
                print(f"Warning: Output shape {resized_img_array.shape} does not match target {target_shape}")

        # Pillow comparison (optional, as it's a fixed bicubic)
        # pillow_resized_img = input_img_pil.resize(golden_img.size, Image.BICUBIC)
    ...
    print("\nModified bicubic variants script tests completed.")
