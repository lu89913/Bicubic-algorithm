...
## 7. Bicubic 變種實驗與 PSNR 提升探索 (`modified_bicubic_variants.py`)

在完成了對 Bicubic 演算法的 LUT 優化和定點化之後，我們進一步探索了是否可以通過調整原始 Bicubic 演算法（即直接計算三次多項式，非 LUT）的核函數參數或使用其變種來獲得更好的 PSNR。
...
本專案成功地完成了對 Bicubic 內插演算法的優化探索（通過 LUT 簡化核心運算）、定點化實現，以及通過調整核參數提升PSNR的實驗。

*   **成果**:
    *   所有開發的演算法版本（優化浮點、定點）在 PSNR 指標上均達到了與傳統實現相當或有所提升的水平，圖像品質得到了保證或改善。特別是 **Keys 核 (a=-0.75) 版本，PSNR 達到 34.4137 dB**。
...
*   **未來工作**:
    ...
    *   **硬體/低階語言實現考量**: 本專案的定點版本為在 C/C++ 或 HDL 中實現該演算法提供了清晰的邏輯。可以進一步考慮如何優化內存訪問、並行性等。對於追求最高 PSNR 的應用，可以考慮將 Keys(a=-0.75) 的版本進行定點化。
